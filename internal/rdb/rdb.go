...
func parseDatabase(reader *bufio.Reader, rdb *RDB) error {
	for {
		...
		err = parseTable(reader, db)

		if err != nil {
			return fmt.Errorf("error parsing table for database %d: %v", idx, err)
		}
	}
}

func parseTable(reader *bufio.Reader, db *Database) error {
	for {
		flag, err := reader.Peek(1)
		if err != nil {
			return err
...
func readEncodedSize(reader *bufio.Reader) (int, error) {
	...
	case 0b_01:
		...
		return int((firstByte&mask)<<8 | secondByte), nil
	case 0b_10:
